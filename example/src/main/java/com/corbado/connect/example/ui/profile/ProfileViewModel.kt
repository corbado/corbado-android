...
class ProfileViewModel(application: Application) : AndroidViewModel(application) {
        ...
        viewModelScope.launch {
            ...
            when (val manageStep = corbado.isManageAppendAllowed(::connectTokenProvider)) {
                ...
                is ConnectManageStep.Error -> {
                    errorMessage.value =
                        "Unable to access passkeys. Check your connection and try again."
                }
            }
    ...
    fun appendPasskey(activityContext: android.content.Context) {
        viewModelScope.launch {
            when (val appendStatus =
                corbado.completePasskeyListAppend(activityContext, ::connectTokenProvider)) {
                is ConnectManageStatus.Done -> {
                    passkeys.value = appendStatus.passkeys
                    listMessage.value =
                        if (appendStatus.passkeys.isEmpty()) "There is currently no passkey saved for this account." else null
                }
                ...
                is ConnectManageStatus.PasskeyOperationExcludeCredentialsMatch -> {
                    errorMessage.value =
                        "You already have a passkey that can be used on this device."
                }
            }
        }
    }
        ...
        viewModelScope.launch {
            when (val deleteStatus = corbado.deletePasskey(::connectTokenProvider, passkeyId)) {
                is ConnectManageStatus.Done -> {
                    ...
                    listMessage.value =
                        if (deleteStatus.passkeys.isEmpty()) "There is currently no passkey saved for this account." else null
                }
    ...
    fun goToHome() {
        viewModelScope.launch {
            _navigationEvents.emit(NavigationEvent.NavigateTo(Screen.Home.route))
        }
    }
    ...
    fun signOut() {
        viewModelScope.launch {
            ...
            _navigationEvents.emit(NavigationEvent.NavigateTo(Screen.Login.route))
        }
    }
} 