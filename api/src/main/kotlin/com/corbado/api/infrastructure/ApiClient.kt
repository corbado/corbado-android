package com.corbado.api.infrastructure
...
open class ApiClient(val baseUrl: String, val client: Call.Factory = defaultClient) {
    companion object {
        ...
        const val baseUrlKey: String = "com.corbado.api.baseUrl"
    ...
    protected fun <T> updateAuthParams(requestConfig: RequestConfig<T>) {
        ...
        if (requestConfig.headers["X-Corbado-ProjectID"].isNullOrEmpty()) {
            if (apiKey["X-Corbado-ProjectID"] != null) {
                if (apiKeyPrefix["X-Corbado-ProjectID"] != null) {
                    requestConfig.headers["X-Corbado-ProjectID"] = apiKeyPrefix["X-Corbado-ProjectID"]!! + " " + apiKey["X-Corbado-ProjectID"]!!
                } else {
                    requestConfig.headers["X-Corbado-ProjectID"] = apiKey["X-Corbado-ProjectID"]!!
                }
            }
        }
    }
