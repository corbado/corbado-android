...
class ConnectExampleUITests {
    ...
    @Test
    fun testInAppAppendInstant() = runTest {
        ...
        // auto-append will create a passkey instantly
        ...
        profileScreen.deletePasskey(profileScreen2.getPasskeyIds()[0], true)
        waitForCondition { profileScreen.countNumberOfPasskeys() == 0 }

        // if auto-append is skipped, the user can append through the sheet
        val homeScreen2 = profileScreen2.goToHome()
        homeScreen2.declineAutomaticAppend(controlServer)
        homeScreen2.acceptBottomSheetAppend()
        val profileScreen3 = homeScreen.navigateBackToProfileScreen()
        waitForCondition { profileScreen.countNumberOfPasskeys() == 1 }
        profileScreen.deletePasskey(profileScreen3.getPasskeyIds()[0], true)
        waitForCondition { profileScreen.countNumberOfPasskeys() == 0 }

        // buttons should not trigger appends for an instant user
        val homeScreen3 = profileScreen3.goToHome()
    ...
    @Test
    fun testInAppAppendButtons() = runTest {
        val initialScreen = startApp(allowInAppAppendButton = true)
        val email = TestDataFactory.createEmail()
        val authenticatorApp = AuthenticatorApp()
    ...
    private suspend fun startApp(
        ...
        allowInAppAppendInstant: Boolean = false,
        allowInAppAppendButton: Boolean = false,
    ): LoginScreen {
        composeTestRule.activity.setInvTokens(allowedByGradualRollout, allowInAppAppendInstant, allowInAppAppendButton)
